 ...
 * @author J4M0
 */
public class ListarSolicitudesBorrador extends SolicitudForm{
    ...
    public ListarSolicitudesBorrador(Usuario usuario) {
        super(usuario);        
    ...
    private void init() {
        ...
        buttons.addComponent(editar);
        buttons.addComponent(eliminar);
        table = new Table();
        table.setSelectable(true);
        table.setImmediate(true);
        table.addContainerProperty("Consecutivo", Long.class,  null);
        table.addContainerProperty("Documento", Long.class,  null);
        ...
        List<Solicitud> solicitud_aux = solicitudes.findByEstado(1);
        if(solicitud_aux.isEmpty()){
            message.setCaption("No tiene solicitudes en tramite");            
            removeAllComponents();
            super.addComponent(super.message);
        } else {
            for(Solicitud s : solicitud_aux){      
                if(s.getEstado().getCodigo()==1){
                    table.addItem(new Object[] {
                        s.getRadicado(),s.getTecnico().getDocumento(),s.getTipoSolicitud().getNombre()} , s);                        
                }
            }
    ...
    private void addListenerT(){
        ...
        editar.addListener(new Button.ClickListener() {
            @Override
    ...
    private void eliminarSolicitud(){
        try{
            ReferenciaPersonalDao rp_aux = new ReferenciaPersonalDao();
            List<ReferenciaPersonal> rp = rp_aux.findBy(new ReferenciaPersonal(solicitud));
            for(ReferenciaPersonal rr : rp){
                rp_aux.delete(rr);
            }
            daoSolicitud.delete(solicitud);
            launchSubwindow("Se elimino la solicitud: " + solicitud.getRadicado());
